...
// import { useState, useEffect } from 'react'
// import Link from 'next/link'
// import { FiPlus, FiEdit, FiTrash2, FiLogOut, FiRefreshCw, FiSearch } from 'react-icons/fi'
// import { useAuth } from '@/lib/hooks/useAuth'
// import { useRouter } from 'next/navigation'
// import { IssueForm } from '@/lib/components/dashboard/IssueForm'
// import { IssueList } from '@/lib/components/dashboard/IssueList'
...
//         status: issue.status.toLowerCase().replace('_', '-') as Status,
//         createdAt: new Date(issue.createdAt).toISOString().split('T')[0],
//       }))
...
>
  ...
  Logout
      ...
      <main className="container mx-auto px-4 py-8">
        {/* STATS */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Stat title="Total Issues" value={issues.length} />
          <Stat
            title="Cloud Security"
            value={issues.filter(i => i.type === 'CLOUD_SECURITY').length}
          />
          <Stat
            title="Red Team"
            value={issues.filter(i => i.type === 'RETEAM_ASSESSMENT').length}
          />
          <Stat
            title="VAPT"
            value={issues.filter(i => i.type === 'VAPT').length}
          />
        </div>
